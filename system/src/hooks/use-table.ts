...
export function useFilter<T extends Record<string, unknown>>({ defaultFilters, defaultSearchQuery = "" }: UseFilterOptions<T>): UseFilterReturn<T> {
...
export function useTable<TSort extends string, TFilters extends Record<string, unknown>>({ sort: sortOptions, pagination: paginationOptions = {}, filter: filterOptions }: UseTableOptions<TSort, TFilters>): UseTableReturn<TSort, TFilters> {
  ...
  }, [filterState.searchQuery, filterState.filters, sortState.sortField, sortState.sortDirection]);
